...
.detailControls{display:flex;align-items:center;gap:10px}.viewToggle{display:flex;padding:3px;border:1px solid var(--line);border-radius:7px;background:#0b1118}.viewToggle button{border:0;background:transparent;color:var(--muted);padding:7px 10px;border-radius:5px;font-size:11px;font-weight:750;cursor:pointer}.viewToggle button.active{background:#1b3b58;color:#dceeff}.yearCalendar{border-top:1px solid var(--line);padding:18px}.calendarNav{display:flex;align-items:center;justify-content:center;gap:18px;margin-bottom:16px}.calendarNav strong{font-size:22px}.calendarNav button{width:34px;height:30px;border:1px solid #37638f;border-radius:6px;background:#123150;color:#dbeeff;cursor:pointer}.monthsGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.monthCard{min-width:0;border:1px solid var(--line);border-radius:8px;background:#0d141c;overflow:hidden}.monthCard h3{margin:0;padding:10px 12px;border-bottom:1px solid var(--line);font-size:14px}.weekdays,.monthDays{display:grid;grid-template-columns:repeat(7,minmax(0,1fr))}.weekdays{padding:6px 7px 3px;color:var(--muted);font-size:9px;text-align:center}.dayCell{position:relative;min-height:65px;padding:5px;border-right:1px solid #1c2732;border-top:1px solid #1c2732;overflow:hidden}.dayCell:nth-child(7n){border-right:0}.emptyDay{background:#0a1016}.dayNumber{display:block;color:var(--muted);font-size:9px;margin-bottom:3px}.dayCell.hasPayments{background:#101d29}.dayPayments{display:flex;flex-direction:column;gap:3px}.calendarPayment{display:block;min-width:0;padding:4px 5px;border-radius:4px;background:#173654;color:#b9dcff;text-decoration:none;font-size:9px;overflow:hidden}.calendarPayment strong,.calendarPayment small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.calendarPayment small{color:#8fb7d9;margin-top:1px}.calendarPayment.past-due{background:#481e22;color:#ffced0}.calendarPayment.due-today{background:#493611;color:#ffe1a5}@media(max-width:1100px){.monthsGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.detailControls{align-items:flex-start;flex-direction:column}}@media(max-width:650px){.monthsGrid{grid-template-columns:1fr}.dayCell{min-height:72px}}
.calendarRange{display:flex;align-items:center;justify-content:center;gap:10px;margin-bottom:16px}.calendarRange strong{font-size:15px}.calendarRange span{color:var(--muted);font-size:11px}.calendarPayment{width:100%;border:0;text-align:left;font:inherit;cursor:pointer}.calendarPayment:hover,.calendarPayment:focus-visible{outline:1px solid #76b9ff;filter:brightness(1.12)}.modalBackdrop{position:fixed;inset:0;z-index:50;display:grid;place-items:center;padding:20px;background:rgba(2,6,10,.78);backdrop-filter:blur(3px)}.modalBackdrop[hidden]{display:none}.paymentModal{width:min(620px,100%);max-height:calc(100vh - 40px);overflow:auto;border:1px solid #3b5269;border-radius:10px;background:#111923;box-shadow:0 24px 80px rgba(0,0,0,.55)}.modalHead{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding:20px 22px 16px;border-bottom:1px solid var(--line)}.modalClose{border:1px solid var(--line);border-radius:6px;background:#0b1118;color:var(--ink);width:32px;height:32px;font-size:20px;cursor:pointer}.paymentDetails{display:grid;grid-template-columns:1fr 1fr;margin:0;padding:6px 22px 16px}.paymentDetails div{padding:13px 0;border-bottom:1px solid #222f3b}.paymentDetails div:nth-child(odd){padding-right:18px}.paymentDetails dt{color:var(--muted);font-size:10px;text-transform:uppercase;letter-spacing:.08em}.paymentDetails dd{margin:4px 0 0;font-size:13px;font-weight:700}.modalActions{display:flex;justify-content:flex-end;gap:9px;padding:16px 22px 20px}.secondaryButton,.squareInvoiceButton{border-radius:7px;padding:10px 13px;font-weight:750;font-size:12px;cursor:pointer}.secondaryButton{border:1px solid var(--line);background:#0b1118;color:var(--ink)}.squareInvoiceButton{border:1px solid #37638f;background:#123150;color:#dbeeff;text-decoration:none}@media(max-width:560px){.paymentDetails{grid-template-columns:1fr}.paymentDetails div:nth-child(odd){padding-right:0}.modalActions{flex-direction:column-reverse}.secondaryButton,.squareInvoiceButton{text-align:center}}
.calendarNotice{grid-column:1/-1;padding:9px 11px;border:1px solid #725d2d;border-radius:6px;background:#2b2414;color:#e7c97e;font-size:11px}
.monthDays{border-bottom:1px solid #1c2732}.calendarPayment{padding:3px 4px;font-family:inherit;font-size:8px;line-height:1.2}.calendarPayment strong{font-size:9px;white-space:nowrap}.calendarPayment small{font-size:7px;white-space:normal;overflow:visible;text-overflow:clip;line-height:1.15;overflow-wrap:anywhere}
</style>
</head>
<body><main>
...
<section class="panel"><div class="head"><div><span class="eyebrow">Payment detail</span><h2>Outstanding payment schedule</h2></div><div class="detailControls"><div class="viewToggle" aria-label="Payment detail view"><button type="button" class="active" id="tableView">Table</button><button type="button" id="calendarView">Calendar</button></div><div class="filters"><input id="search" placeholder="Search invoice or client" aria-label="Search invoices"><select id="timing" aria-label="Filter by timing"><option value="">All payments</option><option>Past due</option><option>Due today</option><option>Future</option></select></div></div></div>
...
<?php foreach($rows as $row): $dueClass=$row['timing']==='Past due'?' past-due':($row['timing']==='Due today'?' due-today':''); ?><tr data-search="<?=h(strtolower($row['invoice_number'].' '.$row['client'].' '.$row['title']))?>" data-timing="<?=h($row['timing'])?>" data-bucket="<?=h($row['bucket'])?>" data-due-date="<?=h($row['due_date'])?>" data-client="<?=h($row['client'])?>" data-title="<?=h($row['title'])?>" data-event-date="<?=h($row['event_date'])?>" data-amount="<?=$row['payment_amount']?>" data-total="<?=$row['invoice_total']?>" data-outstanding="<?=$row['outstanding']?>" data-status="<?=h($row['invoice_status'])?>" data-invoice="<?=h($row['invoice_number'])?>" data-url="<?=h($row['url'])?>"><td data-value="<?=h($row['invoice_number'])?>"><a class="link" href="<?=h($row['url'])?>" target="_blank" rel="noopener"><?=h($row['invoice_number'])?> ↗</a></td><td data-value="<?=h($row['client'])?>"><strong><?=h($row['client'])?></strong><small><?=h($row['title'])?></small></td><td data-value="<?=h($row['event_date'])?>"><?=h(displayDate($row['event_date']))?></td><td data-value="<?=$row['invoice_total']?>"><?=money($row['invoice_total'])?></td><td data-value="<?=$row['outstanding']?>"><?=money($row['outstanding'])?></td><td class="dueCell<?=$dueClass?>" data-value="<?=h($row['due_date'])?>"><strong><?=h(displayDate($row['due_date']))?></strong></td><td data-value="<?=$row['payment_amount']?>"><strong><?=money($row['payment_amount'])?></strong></td><td><span class="badge <?=h(strtolower(str_replace(' ','-',$row['invoice_status'])))?>"><?=h($row['invoice_status'])?></span></td></tr><?php endforeach; ?>
</tbody></table></div><div class="yearCalendar" id="yearCalendar" hidden><div class="calendarRange" id="calendarRange"></div><div class="monthsGrid" id="monthsGrid"></div></div><footer><span id="shown"><?=count($rows)?></span> outstanding payments shown · Amounts reflect unpaid balances as of report time</footer></section>
<div class="modalBackdrop" id="paymentModal" hidden><section class="paymentModal" role="dialog" aria-modal="true" aria-labelledby="modalTitle"><div class="modalHead"><div><span class="eyebrow">Payment information</span><h2 id="modalTitle"></h2></div><button type="button" class="modalClose" id="modalClose" aria-label="Close payment information">×</button></div><dl class="paymentDetails" id="paymentDetails"></dl><div class="modalActions"><button type="button" class="secondaryButton" id="modalCancel">Close</button><a class="squareInvoiceButton" id="modalInvoice" target="_blank" rel="noopener">Open Square invoice ↗</a></div></section></div>
</main><script>
const table=document.querySelector('#payments'),tableWrap=document.querySelector('.tablewrap'),body=table.tBodies[0],search=document.querySelector('#search'),timing=document.querySelector('#timing'),shown=document.querySelector('#shown'),clearGraph=document.querySelector('#clearGraph'),selectionSummary=document.querySelector('#selectionSummary'),selectedTotal=document.querySelector('#selectedTotal'),selectedMeta=document.querySelector('#selectedMeta'),yearCalendar=document.querySelector('#yearCalendar'),monthsGrid=document.querySelector('#monthsGrid'),calendarRange=document.querySelector('#calendarRange'),tableView=document.querySelector('#tableView'),calendarView=document.querySelector('#calendarView'),paymentModal=document.querySelector('#paymentModal'),paymentDetails=document.querySelector('#paymentDetails'),selectedBuckets=new Set(),currency=new Intl.NumberFormat('en-US',{style:'currency',currency:'USD'}),dateFormat=new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'}),monthFormat=new Intl.DateTimeFormat('en-US',{month:'long',year:'numeric',timeZone:'UTC'});
...
function showPayment(row){document.querySelector('#modalTitle').textContent='Invoice '+row.dataset.invoice;const fields=[['Client',row.dataset.client],['Title',row.dataset.title],['Event date',dateFormat.format(new Date(row.dataset.eventDate))],['Payment due',dateFormat.format(new Date(row.dataset.dueDate))],['Payment amount',currency.format(Number(row.dataset.amount)/100)],['Invoice status',row.dataset.status],['Invoice total',currency.format(Number(row.dataset.total)/100)],['Total outstanding',currency.format(Number(row.dataset.outstanding)/100)]];paymentDetails.replaceChildren();fields.forEach(([label,value])=>{const wrap=document.createElement('div'),dt=document.createElement('dt'),dd=document.createElement('dd');dt.textContent=label;dd.textContent=value;wrap.append(dt,dd);paymentDetails.append(wrap)});document.querySelector('#modalInvoice').href=row.dataset.url;paymentModal.hidden=false}
function renderCalendar(){monthsGrid.replaceChildren();const allVisible=[...body.rows].filter(row=>!row.hidden),visible=allVisible.filter(row=>/^\d{4}-\d{2}-\d{2}$/.test(row.dataset.dueDate)&&!Number.isNaN(Date.parse(row.dataset.dueDate+'T00:00:00Z'))).sort((a,b)=>a.dataset.dueDate.localeCompare(b.dataset.dueDate)),undatedCount=allVisible.length-visible.length;calendarRange.replaceChildren();if(!visible.length){const empty=document.createElement('p');empty.className='empty';empty.textContent='No payments with valid due dates match the current filters.'+(undatedCount?' '+undatedCount+' payment'+(undatedCount===1?'':'s')+' cannot be placed because Square did not provide a valid due date.':'');monthsGrid.append(empty);return}const [startYear,startMonth]=visible[0].dataset.dueDate.split('-').map(Number),[endYear,endMonth]=visible[visible.length-1].dataset.dueDate.split('-').map(Number),monthCount=(endYear-startYear)*12+endMonth-startMonth+1,startDate=new Date(Date.UTC(startYear,startMonth-1,1)),endDate=new Date(Date.UTC(endYear,endMonth-1,1)),startLabel=document.createElement('strong'),through=document.createElement('span'),endLabel=document.createElement('strong');startLabel.textContent=monthFormat.format(startDate);through.textContent='through';endLabel.textContent=monthFormat.format(endDate);calendarRange.append(startLabel,through,endLabel);if(undatedCount){const notice=document.createElement('div');notice.className='calendarNotice';notice.textContent=undatedCount+' payment'+(undatedCount===1?'':'s')+' not shown because Square did not provide a valid due date.';monthsGrid.append(notice)}for(let offset=0;offset<monthCount;offset++){const monthDate=new Date(Date.UTC(startYear,startMonth-1+offset,1)),year=monthDate.getUTCFullYear(),month=monthDate.getUTCMonth(),card=document.createElement('section'),heading=document.createElement('h3'),weekdays=document.createElement('div'),days=document.createElement('div'),first=monthDate.getUTCDay(),count=new Date(Date.UTC(year,month+1,0)).getUTCDate();card.className='monthCard';heading.textContent=monthFormat.format(monthDate);weekdays.className='weekdays';['S','M','T','W','T','F','S'].forEach(label=>{const span=document.createElement('span');span.textContent=label;weekdays.append(span)});days.className='monthDays';for(let blank=0;blank<first;blank++){const cell=document.createElement('div');cell.className='dayCell emptyDay';days.append(cell)}for(let day=1;day<=count;day++){const date=year+'-'+String(month+1).padStart(2,'0')+'-'+String(day).padStart(2,'0'),payments=visible.filter(row=>row.dataset.dueDate===date),cell=document.createElement('div'),number=document.createElement('span'),list=document.createElement('div');cell.className='dayCell'+(payments.length?' hasPayments':'');number.className='dayNumber';number.textContent=day;list.className='dayPayments';payments.forEach(row=>{const button=document.createElement('button'),amount=document.createElement('strong'),client=document.createElement('small');button.type='button';button.className='calendarPayment '+row.dataset.timing.toLowerCase().replaceAll(' ','-');button.title='View '+row.dataset.client+' payment details';button.addEventListener('click',()=>showPayment(row));amount.textContent=currency.format(Number(row.dataset.amount)/100);client.textContent=row.dataset.client;button.append(amount,client);list.append(button)});cell.append(number,list);days.append(cell)}const trailing=(7-((first+count)%7))%7;for(let blank=0;blank<trailing;blank++){const cell=document.createElement('div');cell.className='dayCell emptyDay';cell.setAttribute('aria-hidden','true');days.append(cell)}card.append(heading,weekdays,days);monthsGrid.append(card)}}
function filter(){let n=0,q=search.value.trim().toLowerCase();[...body.rows].forEach(r=>{let graphOk=selectedBuckets.size===0||selectedBuckets.has(r.dataset.bucket),ok=(!q||r.dataset.search.includes(q))&&(!timing.value||r.dataset.timing===timing.value)&&graphOk;r.hidden=!ok;if(ok)n++});shown.textContent=n;clearGraph.hidden=selectedBuckets.size===0;clearGraph.textContent=selectedBuckets.size?'Clear graph filters ('+selectedBuckets.size+')':'Clear graph filters';updateSelectionSummary();if(!yearCalendar.hidden)renderCalendar()}
...
tableView.addEventListener('click',()=>{tableWrap.hidden=false;yearCalendar.hidden=true;tableView.classList.add('active');calendarView.classList.remove('active')});calendarView.addEventListener('click',()=>{tableWrap.hidden=true;yearCalendar.hidden=false;calendarView.classList.add('active');tableView.classList.remove('active');renderCalendar()});function closePayment(){paymentModal.hidden=true}document.querySelector('#modalClose').addEventListener('click',closePayment);document.querySelector('#modalCancel').addEventListener('click',closePayment);paymentModal.addEventListener('mousedown',event=>{if(event.target===paymentModal)closePayment()});
